...
function resolveStaticCommand(commandName) {
...
function parseRegexTemplate(template) {
    try {
...
async function resolveCommand(commandName, args, aliasScope) {
    ...
    const alias = await db.query.commandAliases.findFirst({
        where: (alias, { and, eq }) => and(eq(alias.alias, commandName), eq(alias.scopeType, aliasScope.scopeType), eq(alias.scopeId, aliasScope.scopeId))
    }) ?? await db.query.commandAliases.findFirst({
        where: and(eq(commandAliases.alias, commandName), eq(commandAliases.scopeType, 'global'), isNull(commandAliases.scopeId))
    });
    if (!alias) {
...
async function sendReply(client, data, text) {
...
async function checkCooldown(client, data, commandName, commandCooldown) {
    ...
    const members = await client.getGroupMemberList(data.group_id);
...
async function handleMessage(client, data) {
    ...
    if (command.validateArgs) {
        const validateResult = await command.validateArgs(resolvedArgs, client, data);
    ...
    try {
        await command.execute(resolvedArgs, client, data);
    }
    catch (error) {
