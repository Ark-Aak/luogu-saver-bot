import { resolveCommandUsage } from '.';
    ...
    async execute(args, client, data) {
        ...
        if (action === 'list') {
            const aliases = await db.query.commandAliases.findMany({
                where: (alias, { or, and, eq, isNull }) => or(and(eq(alias.scopeType, scopeType), eq(alias.scopeId, scopeId)), and(eq(alias.scopeType, 'global'), isNull(alias.scopeId)))
            });
            if (aliases.length === 0) {
        ...
        if (action === 'set' || action === 'setglobal') {
            ...
            const existing = await db.query.commandAliases.findFirst({
                where: (alias, { and, eq, isNull }) => and(eq(alias.alias, aliasName), eq(alias.scopeType, actualScopeType), actualScopeId === null ? isNull(alias.scopeId) : eq(alias.scopeId, actualScopeId))
            });
            if (existing) {
