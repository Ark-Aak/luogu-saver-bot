    ...
    async execute(args, client, data) {
        ...
        if (action === 'list') {
            const aliases = await db.query.commandAliases.findMany({
                where: (alias, { or, and, eq }) => or(and(eq(alias.scopeType, scopeType), eq(alias.scopeId, scopeId)), and(eq(alias.scopeType, 'global'), eq(alias.scopeId, 0)))
            });
            if (aliases.length === 0) {
        ...
        if (action === 'set' || action === 'setglobal') {
            ...
            const existing = await db.query.commandAliases.findFirst({
                where: (alias, { and, eq }) => and(eq(alias.alias, aliasName), eq(alias.scopeType, actualScopeType), eq(alias.scopeId, actualScopeId))
            });
            if (existing) {
